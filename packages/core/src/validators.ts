...
export class NotEmptyObjectViolation extends Violation {
  constructor(invalidValue: any, path: Path, public optionalProperties?: string[]) {
    super(path, 'NotEmptyObject', invalidValue);
    if (!optionalProperties) {
      delete this.optionalProperties;
    }
  }
}
...
export const defaultViolations = {
  ...
  notEmptyObject: (invalidValue: any, path: Path = ROOT, optionalProperties?: string[]) => new NotEmptyObjectViolation(invalidValue, path, optionalProperties),
...
export class NotEmptyObjectValidator extends Validator {
  constructor(public readonly optionalProperties?: string[]) {
    super();
  }

  validatePath(value: any, path: Path, ctx: ValidationContext): PromiseLike<ValidationResult> {
    return !isNullOrUndefined(value) && !isEmptyObject(value)
      ? ctx.successPromise(value)
      : ctx.failurePromise(defaultViolations.notEmptyObject(value, path, this.optionalProperties), value);
  }
}
